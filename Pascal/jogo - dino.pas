...
const dino_col_hor = 4; // tamanho de colisao horizontal do dino
const dino_col_ver = 4; // tamanho de colisao vertical do dino
...
const max_detalhes = 4; // numero maximo de detalhes
...
var
	...
	// as matrizes foram usadas para fazer a parte visual do jogo
	// posicoes dos detalhes do jogo, obstaculos, nuvens, etc
	detalhes_posicao:array[1..max_detalhes, 1..2] of real; // matriz do tipo real para permitir movimentos com numeros quebrados
	detalhes_altura:array[1..max_detalhes, 1..2] of integer; // do tipo integer pois como nao ha movimento vertical, nao ha necessidade de ser numero quebrado 
	// detalhes_x[id, 1] = nuvens
	// detalhes_x[id, 2] = "pedras" no chao
...
begin
	...
	// desenho da linha horizontal
	linha := '';
	for i := 1 to mapa_largura do	
		linha := linha + #254;
	write(linha);
	               
	// desenho das nuvens   // desenha do fim ao inicio para impedir alguns bugs visuais quando uma nuvem esta acima da outra
	for i := max_detalhes downto 1 do
	begin
		// a altura da nuvem tambem decide a velocidade horizontal e o tamanho dela
		tipo_nuvem := detalhes_altura[i,1];
		detalhes_posicao[i,1] := detalhes_posicao[i,1] + (velocidade / (4-tipo_nuvem)* 0.5);
		DesenharNuvem(mapa_largura-round(round(detalhes_posicao[i,1]) mod mapa_largura), 7 - tipo_nuvem, tipo_nuvem);
	end;

	// desenhos das pedrinhas no chao
	for i := 1 to max_detalhes do
	begin
		detalhes_posicao[i,2] := detalhes_posicao[i,2] + velocidade;
		textcolor(cor_chao);
		gotoxy(mapa_largura - (round(detalhes_posicao[i,2])) mod mapa_largura+1, altura_chao+detalhes_altura[i,2]);
		write(#254);
	end;
end;
...
begin
	...
	// gera um padrao de cenario novo
	for i := 1 to max_detalhes do   
		for j := 1 to 2 do
		begin
			detalhes_posicao[i,j] := random(mapa_largura);
			detalhes_altura[i,j] := random(3)+1;
		end;
	clrscr;
end;
...
begin
	...
	begin
		...
		ApagarDesenho(1,1,mapa_largura+20,19);
